...
export default function Show({ document, assistant }) {
    ...
    const onCitation = (ref) => {
        if (!ref) return;
        const targetPage = ref.page || 1;
        // Inject targetPage ke dalam ref agar highlight.page tidak kosong
        setHighlight({ ...ref, page: targetPage });
        setPage(targetPage);
    };
        ...
        <div className="flex h-screen flex-col bg-[#f5f7fa]">
            ...
            <header className="relative z-30 border-b border-slate-200/70 bg-white shadow-sm">
                <div className="flex items-center gap-2 px-3 py-2.5 sm:gap-3 sm:px-4">
                    <Link
                        ...
                        title="Kembali ke detail dokumen"
            ...
            <div className={`flex flex-1 overflow-hidden ${isMobile ? 'flex-col' : 'flex-row'}`}>
                <div className="min-h-0 min-w-0 flex-1">
                    <PdfViewer 
                        url={`/documents/${document.id}/pdf`} 
                        page={page} 
                        highlight={highlight} 
                        onPageChange={setPage} 
                        onSelectText={(text) => {
                            setChatOpen(true);
                            setSelectedTextForChat(`Tolong jelaskan mengenai kalimat ini:\n"${text}"\n`);
                        }}
                    />
                </div>
                    ...
                    <>
                        ...
                        >
                            <div className="h-full" style={{ width: chatWidth }}>
                                <ChatPanel documentId={document.id} onCitation={onCitation} onClose={() => setChatOpen(false)} assistant={assistant} initialInput={selectedTextForChat} clearInitialInput={() => setSelectedTextForChat("")} />
                            </div>
                        </div>
                    </>
                )}
